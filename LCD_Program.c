...
if (LOC_u8Number == 0) {
  ...
  // if number is greater than 0 and less than or equal to 15, print the corresponding hex digit
  else if (LOC_u8Number > 0 && LOC_u8Number <= 15) {
    LCD_VidWriteChar(LOC_u8Number - 1 + '1');
  } else {
    char hexChars[10];  // Assuming a maximum of 10 characters for the hex representation
    u8 i = 0;

    // if number is not between 1 and 15, print it as a hexadecimal digit
    while (LOC_u8Number != 0) {
      u32 remainder = LOC_u8Number % 16;
      if (remainder < 10) {
        hexChars[i++] = remainder + '0';
      } else {
        hexChars[i++] = remainder - 10 + 'A';
      }
      LOC_u8Number /= 16;
    }
	...
	/*if (LOC_u8Number == 0)
	{
		LCD_VidWriteChar(LOC_u8Number + 0x30);
	}
	else
	{
		// if we don't reserve the number, the number will be written
		...
		while (LOC_u8Number != 0)
		{
			reverse = (reverse * 16) + (LOC_u8Number % 16);
		...
		while (reverse != 0)
		{
			LCD_VidWriteChar(reverse % 16 + 0x30);
			reverse /= 16;
		}
	}*/
}
...
void LCD_spe_char (u8 Row , u8 Col)
{
	...
	LCD_vidWriteExtraChar (Row,Col, 8);
	_delay_ms(250);
	LCD_VidSendCommand(0b00000001);
	_delay_ms(2);
	LCD_vidWriteExtraChar (Row,Col, 8);
